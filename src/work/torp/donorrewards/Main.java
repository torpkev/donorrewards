package work.torp.donorrewards;
...
import work.torp.donorrewards.commands.Donor;
...
public class Main extends JavaPlugin {
    ...
    public void AddDonation(Donation d)
    {
    	try {
			...
			Alert.DebugLog("Main", "AddDonation", "Unexpected Error - " + ex.getMessage());  
		}
    } 
    public void RemoveDonation(Donation d)
    {
    	try {
	    	List<Donation> lstD = new ArrayList<Donation>();
	    	if (d != null)
	    	{
	    		if (donations != null) {
	    			for (Donation don: donations)
		    		{
	    				if (don.getDonationID() != don.getDonationID())
		    			{
	    					lstD.add(don);
		    			}
		    		}
			...
			Alert.DebugLog("Main", "RemoveDonation", "Unexpected Error - " + ex.getMessage());  
		}
    }
    ...
    public void loadCommands() {
    	Alert.DebugLog("Main", "loadCommands", "Activating /donor");
		try {
    ...
    public void loadFromDatabase() {
    	Alert.Log("Main.loadFromDatabase", "Getting Donations");
    	this.db.getDonations();
    	if (donations != null)
    	{
    		Alert.DebugLog("Main", "loadFromDatabase", "# Donations: " + Integer.toString(donations.size()));
    	} else {
	...
	public void onEnable() {
    	...
    	try {
			...
			saveDefaultConfig();
			Alert.Log("Main", "Starting Donor Rewards");
