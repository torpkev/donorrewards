package work.torp.donorrewards.alerts;
import java.io.File;
...
public class Alert {
	public static void Sender(String message, CommandSender sender, boolean includeHeader) {
		...
		if (includeHeader)
		{
			header = ChatColor.BLUE + "[Donor Rewards] ";
		}
		if (sender != null)
		{
			if (message != null) {
	...
	public static void Log(String function, String message)
	{
		...
		clogger.sendMessage(ChatColor.DARK_PURPLE + "[Donor Rewards]" + ChatColor.GOLD + "[" + function + "] " + ChatColor.WHITE + message);
	}
	public static void VerboseLog(String function, String message)
	{
		...
		if (vlog != null)
		{
			if (vlog.toLowerCase() == "true") {
				ConsoleCommandSender clogger = Main.getInstance().getServer().getConsoleSender();
				clogger.sendMessage(ChatColor.DARK_RED + "[Donor Rewards.Verbose]" + ChatColor.GOLD + "[" + function + "] " + ChatColor.WHITE + message);
			}	
		}
	}
	public static void DebugLog(String function, String subfunction, String message)
	{
		...
		if (elog != null)
		{
			if (elog.toLowerCase() == "true") {
				ConsoleCommandSender clogger = Main.getInstance().getServer().getConsoleSender();
				clogger.sendMessage(ChatColor.LIGHT_PURPLE + "[Donor Rewards.Debug]" + ChatColor.AQUA + "[" + function + "." + subfunction + "] " + ChatColor.WHITE + message);
				if (Main.getInstance().getDebugFile()) {
